...
for(int j = 0; j< 2*vWhy;j++){
  ...
  int rand1 = floor(random(0, 2));
  for(int i = 0; i<=vHex;i++){
   line(2*i*wid + rand1*wid, j*hei + i*hei + rand1*0.5*hei, 2*i*wid + wid + rand1*wid, j*hei + i*hei + 0.5*hei + rand1*0.5*hei + hei);
  }
}
for(int j = 0; j< 2*vWhy;j++){
  int rand1 = floor(random(0, 2));
  for(int i = 0; i<=vHex;i++){
   line(width - 2*i*wid - rand1*wid, j*hei + i*hei + rand1*0.5*hei + 0.5*hei, width -  2*i*wid - wid - rand1*wid, j*hei + i*hei + 0.5*hei + rand1*0.5*hei + 0.5*hei + hei);
    }
  }
// endRecord();
